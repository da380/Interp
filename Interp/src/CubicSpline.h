...
CubicSpline<xIter, yIter>::CubicSpline(xIter xS, xIter xF, yIter yS,
    ...
    : xS{xS}, xF{xF}, yS{yS} {
     ...
     //  ypp = Vector(n);
     if (left == CubicSplineBC::Free) {
          yRHS(0) = static_cast<y_value_t>(0);
     } else {
          yRHS(0) = (yS[1] - yS[0]) / (xS[1] - xS[0]) - ypl;
     }
     for (int i = 1; i < n - 1; i++) {
          yRHS(i) = (yS[i + 1] - yS[i]) / (xS[i + 1] - xS[i]) -
                    (yS[i] - yS[i - 1]) / (xS[i] - xS[i - 1]);
     }
     if (right == CubicSplineBC::Free) {
          yRHS(n - 1) = static_cast<y_value_t>(0);
     } else {
          yRHS(n - 1) = (yS[n - 1] - yS[n - 2]) / (xS[n - 1] - xS[n - 2]) - ypr;
     }

     // Solve the linear system.
     Eigen::BiCGSTAB<Matrix> solver;
     solver.compute(A);
     ypp = solver.solve(yRHS);
     //  ypp = temp;
